...
public:
                 ...
                 pocket *vCache)
    {
        ...
        */
        ...
        transferPocket(attn->forward(attInp, sinTheta, cosTheta, crntSeqSize, requiredOps, kCache, vCache), attInp);
        // attInp->printShape();
...
class llama
{
    ...
    int qkvDim = 128, maxSeqLen = 0, numOflayers = 28;
...
public:
    llama(int batchSize, int _maxSeqLen, device _dvc = cpu) : maxSeqLen(_maxSeqLen), dvc(_dvc)
    {
        /*
    ...
    int AddCommand(std::vector<pocket *> &kCache, std::vector<pocket *> &vCache, std::unordered_map<std::string, std::string> cmnd, int modelIndex)
    {
        /*
            -> This function update and initialize the kCache and vCache.
        */
        ...
        */
        ...
        for (int i = 0; i < 28; i++)
        {
            layer[i].forward(inp, sinTheta, cosTheta, 0, requiredOps, kCache[modelIndex * numOflayers + i], vCache[modelIndex * numOflayers + i]);
        }
    ...
    int ask(std::unordered_map<std::string, std::string> que, int &crnSeqLen, std::vector<pocket *> &kCache, std::vector<pocket *> &vCache, int modelIndex)
    {
        /*
        ...
        */
        ...
        for (int i = 0; i < 28; i++)
        {
            layer[i].forward(inp, sinTheta, cosTheta, crnSeqLen, requiredOps, kCache[modelIndex * numOflayers + i], vCache[modelIndex * numOflayers + i]);
        }
    ...
    std::string GetNext(std::vector<pocket *> &kCache, std::vector<pocket *> &vCache, int &crnSeqLen, int &lastGeneratedEle, int modelIndex)
    {
...
class MultiAgent
{
    std::vector<pocket *> kCache, vCache;
    int maxSeqSize, numberOfAgent;
    std::vector<int> *crnSeqLen, *resetSeqLen, *lastGeneratedEle;
    ggml_backend *kvCacheMem;
    llama *brain;

public:
    MultiAgent(int _numberOfAgent, int _maxSeqSize, int batchSize, device dvc) : maxSeqSize(_maxSeqSize), numberOfAgent(_numberOfAgent)
    {
        kvCacheMem = new ggml_backend(50, dvc); // 10MB
        brain = new llama(batchSize, maxSeqSize, dvc);
        ...
        for (int i = 0; i < numberOfAgent; i++)
        {
            for(int j = 0; j < 28; j++)
            {
                kCache.push_back(InitPocket({128, 8, maxSeqSize, batchSize}, kvCacheMem, GGML_TYPE_F32));
                vCache.push_back(InitPocket({128, 8, maxSeqSize, batchSize}, kvCacheMem, GGML_TYPE_F32));
            }
        }

        kvCacheMem->AllocMemory();
        crnSeqLen = new std::vector<int>(numberOfAgent, 0);
        lastGeneratedEle = new std::vector<int>(numberOfAgent, -1);
        resetSeqLen = new std::vector<int>(numberOfAgent, 0);
    }

    void AddCommand(std::vector<std::unordered_map<std::string, std::string>> cmnd)
    {
        for (int i = 0; i < numberOfAgent; i++)
        {
            crnSeqLen->at(i) = brain->AddCommand(kCache, vCache, cmnd[i], i);
        }

        // Set resetSeqLen to size of each cmnd length
        std::copy(crnSeqLen->data() , crnSeqLen->data() + numberOfAgent, resetSeqLen->begin());
    }

    void ask(std::unordered_map<std::string, std::string> que, int agentId)
    {
        lastGeneratedEle->at(agentId) = brain->ask(que, crnSeqLen->at(agentId), kCache, vCache, agentId);
    }

    std::string GetNext(int agentId)
    {
        return brain->GetNext(kCache, vCache, crnSeqLen->at(agentId), lastGeneratedEle->at(agentId), agentId);
    }

    void reset(int i)
    {
        crnSeqLen->at(i) = resetSeqLen->at(i);
    }
