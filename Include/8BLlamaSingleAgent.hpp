...
public:
    ...
    pocket *forward(pocket *inp, pocket *sinTheta, pocket *cosTheta, int64_t crntSeqSize, std::vector<pocket *> &requiredOps)
    {
        ...
        */
        ...
        // Dim: (qkvDim/2) X 1 X 32 X b*s
        pocket *x = slice(q, {0, 0, 0, 0}, {(qkvDim / 2), 1, 32, (b * s)});
        pocket *y = slice(q, {0, 1, 0, 0}, {(qkvDim / 2), 2, 32, (b * s)});

        transferPocket(reshape(x, {(qkvDim / 2), 32, s, b}), x);
        transferPocket(reshape(y, {(qkvDim / 2), 32, s, b}), y);
        ...
        // q Out Dim: qkvDim X 32 X s X b

        // copypocket(q, xNew, {0, 0, 0, 0}, {(qkvDim / 2), 32, s, b}, requiredOps);
        // copypocket(q, yNew, {(qkvDim / 2), 0, 0, 0}, {qkvDim, 32, s, b}, requiredOps);
        ...
        */
        ...
        // qkvDim X 32 X s X b -> qkvDim X s X 32 X b
...
class llama
{
    pocket *sinTheta, *cosTheta, *embd, *remsNorm, *decode;
...
public:
    llama(int batchSize, int _maxSeqLen, std::unordered_map<std::string, std::string> cmnd, device _dvc = cpu) : maxSeqLen(_maxSeqLen), dvc(_dvc)
    {
        ...
        */
        std::vector<int64_t> queryOutSize = {4096, 4096},
                             keyValueSize = {4096, 1024},
                             
                             downSize = {14336, 4096},
                             upSize = {4096, 14336},
                             gateSize = {4096, 14336},
                             inputPostRMSNormSize = {4096},
                             embdSize = {4096, 128256};
        ...
        decode = InitPocket(embdSize, weightMem, GGML_TYPE_F32);
        ...
        Loadpocket("lm_head.weight.bin", decode);
        ...
        if (cmnd["text"].size() != 0)
        {
            ...
            */
            ...
            // include batch
            transferPocket(reshape(inp, {4096, static_cast<int64_t>(index.size()), 1}), inp);
    ...
    void ask(std::unordered_map<std::string, std::string> que)
    {
        ...
        */
        ...
        std::cout << "Ask 2" << std::endl;
        transferPocket(RMSNorm(inp, remsNorm, 1e-5), inp);
        std::cout << "Ask 3" << std::endl;
        inp->printShape();
        decode->printShape(); 
        transferPocket(matmul(inp, decode), inp);

        std::cout << "Ask 4" << std::endl;
    ...
    std::string GetNext()
    {
        ...
        transferPocket(reshape(inp, {4096, 1, 1}), inp);
